...
function Projects() {
  ...
  return (
    <div className="min-h-screen px-6 md:px-20 py-20 border-t border-gray-700/30 backdrop-blur-sm">
      <div className="mb-12">
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-20">
        {featuredProjects.map((project) => (
          <div
            key={project.title}
            className="group border border-gray-700/50 p-6 rounded-xl hover:border-gray-300/60 transition duration-300 bg-gradient-to-br from-gray-900/50 to-gray-800/30 hover:from-gray-800/70 hover:to-gray-700/50 hover:shadow-glow backdrop-blur-sm"
          >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center w-10 h-10 rounded-full border border-gray-600 text-white transition duration-300 group-hover:border-white hover:bg-white hover:text-black hover:shadow-glow"
            >
              <FaGithub size={18} />
            </a>
          </div>
      ...
      <div className="relative px-12 sm:px-14 mb-20">
        <button
          onClick={scrollLeft}
          disabled={index === 0}
          className="absolute left-2 sm:left-0 top-1/2 -translate-y-1/2 bg-black border border-gray-500 px-3 py-2 z-10 hover:bg-white hover:text-black hover:border-white transition disabled:opacity-30 disabled:cursor-not-allowed shadow-glow text-white font-bold"
        >
          ←
        </button>
        ...
        <div
          ...
          style={{ maxWidth: `${visibleCards * 220 + (visibleCards - 1) * 16}px` }}
        >
          <div
          ...
          >
            {miniProjects.map((project) => (
              <div
                key={project.title}
                className="w-[220px] shrink-0 border border-gray-700/50 p-4 rounded-xl hover:scale-105 transition bg-gradient-to-br from-gray-900/50 to-gray-800/30 hover:shadow-glow hover:border-white/60 group backdrop-blur-sm"
              >
                <div className="overflow-hidden rounded-lg mb-3 relative h-28">
                  <img
                    src={project.image}
                    alt={project.title}
                ...
                <div className="flex justify-end mt-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-gray-600 text-white transition duration-300 hover:border-white hover:bg-white hover:text-black"
                  >
                    <FaGithub size={14} />
                  </a>
                </div>
          ...
          <button
            ...
            className="absolute right-2 sm:right-0 top-1/2 -translate-y-1/2 bg-black border border-gray-500 px-3 py-2 z-10 hover:bg-white hover:text-black hover:border-white transition disabled:opacity-30 disabled:cursor-not-allowed shadow-glow text-white font-bold"
          >
            →
          </button>
        </div>
